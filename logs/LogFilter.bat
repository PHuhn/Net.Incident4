...
@REM
 ...
 @ECHO ====================================
 @ECHO
 grep -vE "^\[DHCP IP: \(192.168.1|^\[Service blocked: ICMP_echo_req|^\[Time synchronized|^\[Internet connected|^\[Internet disconnected|^\[admin login|^\[Initialized, firmware|^\[Log Cleared|^\[WLAN access rejected: " %inp% > %1%-probe.txt
 @REM
 TYPE %1%-admin.txt
 TYPE %1%-probe.txt
@GOTO END
